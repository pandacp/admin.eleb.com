...
@section('contents')
    @include('_error')
    <div class="container">
        <form action="{{ route('event_prizes.update',[$event_prize]) }}" method="post">
            <div class="form-group">
                <label for="exampleInputEmail1">奖品名称</label>
                <input type="text" name="name" class="form-control" id="exampleInputEmail1" placeholder="奖品名称" value="{{ $event_prize->name }}">
            </div>
            ...
            <div class="form-group">
                ...
                <select name="events_id" id="" >
                    @foreach($events as $event)
                        <option value="{{ $event->id }}" @if($event->id==$event_prize->events_id) selected @endif>{{ $event->title }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
            ...
            {{ csrf_field() }}
            {{ method_field('PATCH') }}
            <button type="submit" class="btn btn-primary">修改奖品</button>
        </form>
    </div>
