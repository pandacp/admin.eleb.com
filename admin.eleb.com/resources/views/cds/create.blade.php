    ...
    <div class="container">
        <form action="{{ route('cds.store') }}" method="post">
            <div class="form-group">
                ...
                <input type="text" name="name" class="form-control" id="exampleInputEmail1" placeholder="菜单名" value="{{ old('name') }}">
            </div>
            <div class="form-group">
                <label for="exampleInputPassword1">地址</label>
                <input type="text" name="url" class="form-control" id="exampleInputPassword1" placeholder="地址">
            </div>
            <div class="form-group">
                <label for="exampleInputPassword1">关联权限ID</label><br>
                <select name="permission_id" id="">
                    @foreach($permissions as $permission)
                        <option value="{{ $permission->id }}" @if($permission->name=='null') selected @endif>{{ $permission->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
